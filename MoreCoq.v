...
Proof.
  intros n m o p eq1 eq2. 
  rewrite (eq2 n m). reflexivity. rewrite eq1. reflexivity. Qed.
...
Proof.
  intros n.
  destruct n. reflexivity. simpl. intros H. inversion H.
Qed.
(** [] *)
...
Proof.
  ...
  Case "n = 0". intros m H. simpl in H. 
    destruct m. reflexivity. rewrite <- plus_n_Sm in H.
    inversion H.
  Case "n = S m".
    intros m H. rewrite <- plus_n_Sm in H. simpl in H.
    destruct m. inversion H. rewrite <- plus_n_Sm in H. inversion H.
    apply IHn' in H1. apply f_equal. apply H1.
Qed.
(** [] *)
...
Proof.
  ...
  Case "n = S n'". intros eq. destruct m as [| m'].
    ...
    SCase "m = S m'".  apply f_equal.
      simpl in eq. simpl in IHn'. inversion eq. 
...
Proof.
  ...
  Case "n = S n'".
    ...
    intros. apply f_equal. apply IHn'. apply H.
Qed.
...
Proof.
  ...
  Case "l = []". 
    intros n eq. simpl. simpl in eq. rewrite <- eq. reflexivity.
  Case "l = v' :: l'". 
...
Proof.
  ...
  Case "h :: t".
    intros n H.
    inversion H. simpl. apply IHt. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  generalize dependent n.
  induction l as [|h t].
  Case "[]". intros. rewrite <- H. reflexivity.
  Case "h :: t".
    simpl. rewrite <- IHt.
    SCase "implication". intros. rewrite <- H. reflexivity.
    SCase "premise". reflexivity.
Qed.
...
Proof.
  intros X l1 l2 x.
  induction l1 as [|h t].
  Case "[]". intros n H. rewrite <- H. reflexivity.
  Case "h :: t".
    simpl. intros. destruct n. 
    SCase "n = 0". inversion H.
    SCase "n = S n'".
      apply f_equal. apply IHt. inversion H. reflexivity.
Qed.
...
Theorem app_length_plus_distrib : 
  forall (X:Type) (l:list X) (m:list X), length (l ++ m) = length l + length m.
Proof.
  intros.
  induction l as [|h t].
  Case "[]". reflexivity.
...
Proof.
  ...
  Case "[]". intros. rewrite <- H. reflexivity.
  Case "h :: t".
    intros. rewrite app_length_plus_distrib.
...
Theorem double_induction: forall (P : nat -> nat -> Prop), 
  ...
  forall m n, P m n.
Proof.
  intros P A X Y XY m n.
  generalize dependent n.
  induction m as [|m'].
  Case "m = 0". induction n as [|n'].
    SCase "n = 0". apply A.
    SCase "n = S n'". apply Y. apply IHn'.
  Case "m = S m'". destruct n.
    SCase "n = 0". apply X. apply IHm'.
    SCase "n = S n'". apply XY. apply IHm'.
Qed.
...
Proof.
  intros. unfold override. destruct (beq_nat k1 k2). reflexivity. reflexivity.
Qed.
(** [] *)
...
Proof.
  ...
  destruct (beq_nat k1 k2) eqn:eq.
    Case "true". apply beq_nat_true in eq. rewrite <- eq. symmetry. apply H.
...
Proof.
  intros n.
  induction n as [|n'].
    Case "0". intros. destruct m. reflexivity. reflexivity.
    Case "S n'". intros. destruct m. reflexivity. simpl. apply IHn'.
Qed.
(** [] *)
...
Proof.
  ...
  intros X Y l1.
  induction l1 as [|h t]. 
    Case "[]". intros. simpl. destruct l2. reflexivity. inversion H.
    Case "h :: t". intros. simpl. destruct l2 as [|x xs].
      SCase "[]".  inversion H.
      SCase " x :: xs". simpl. rewrite IHt.
        SSCase "implication". reflexivity.
        SSCase "premise". inversion H. reflexivity.
Qed.
(** [] *)
...
Proof.
  intros. unfold override. destruct (beq_nat k1 k3) eqn:k1k3.
    Case "k1 = k3".
      apply beq_nat_true in k1k3. rewrite <- k1k3. rewrite H. reflexivity.
...
Proof.
  ...
  induction l as [|h t].
    Case "[]". inversion H.
    Case "h :: t". simpl in H. destruct (test h) eqn:test_h.
      SCase "true". inversion H. rewrite <- H1. apply test_h.
      SCase "false". apply IHt. apply H.
Qed.  
...
Fixpoint forallb {X:Type} (f:X -> bool) (l:list X) : bool :=
  match l with
    | []     => true
    | x :: t => andb (f x) (forallb f t)
  end.

Fixpoint existsb {X:Type} (f:X -> bool) (l:list X) : bool :=
  match l with
    | []     => false
    | x :: t => orb (f x) (existsb f t) 
  end.

Definition forallb' {X:Type} (f:X -> bool) (l:list X) : bool :=
  fold (fun e acc => andb (f e) acc) l true.

Definition existsb' {X:Type} (f:X -> bool) (l:list X) : bool :=
  negb (forallb (fun e => negb (f e)) l).

Theorem neg_orb_andb : forall (x y:bool), negb (andb x y) = orb (negb x) (negb y).
Proof.
...
Proof.
  ...
  Case "[]". reflexivity.
  Case "h :: t".
    simpl. rewrite <- IHt. unfold existsb'. simpl. rewrite neg_orb_andb. 

...
Case "b = false".
  ...
  SCase "c = false". intros H. rewrite H. reflexivity.
Qed.
...
Proof. 
  ...
  Case "n = 0". reflexivity.
  Case "n = S n'".
    simpl. intros x. rewrite IHn'. reflexivity.
Qed.
...
Proof. 
  ...
  Case "m = 0". rewrite mult_0_r. reflexivity.
  Case "m = S n'". 
    simpl. rewrite IHm'.
    rewrite <- mult_n_Sm.
    reflexivity.
...
induction n as [|n'|n'].
Case "n = 0". reflexivity.
Case "n = T n'". reflexivity.
Case "n = ST n'".
  simpl. rewrite IHn'. simpl.
  rewrite plus_0_r. rewrite plus_n_Sm. rewrite plus_comm.
  reflexivity.
Qed.
...
Proof.
  ...
  Case "n = 0". reflexivity.
  Case "n = S n'". 
    simpl. rewrite incr_unary_same. rewrite IHn'. reflexivity.
Qed.
(** [] *)
...
Fixpoint isPositive (b : binary) : bool :=
  match b with
    | OB   => false
    | T x  => isPositive x
    | ST _ => true
...
Theorem is_positive_incr : forall b : binary, isPositive (incr b) = true.
Proof.
  intros b.
  induction b as [|b'|b'].
  Case "b = OB". reflexivity.
  Case "b = T b'". reflexivity.
  Case "b = ST b'". simpl. rewrite IHb'. reflexivity.
Qed.

Fixpoint normalize (b : binary) : binary :=
  match b with
    | OB   => OB
    | T  x => match isPositive x with
      | false => OB
      | true  => T (normalize x)
      end
    | ST x => ST (normalize x)
...
Fixpoint isNormalized (n : binary) : bool := 
  match n with
    | OB       => true
    | T OB     => false
    | T x      => isNormalized x
...
Theorem twice_reverse_normality : 
  forall n : binary, (isNormalized (T n) = true) -> (isNormalized n = true).
Proof.
  intros n H.
  destruct n as [|b|b].
  Case "n = OB". reflexivity.
  Case "n = T b".  
...
Theorem incr_preserves_normality : 
  forall n : binary, (isNormalized n = true) -> (isNormalized (incr n) = true).
Proof.
  intros n H. induction n as [|b|b].
  Case "n = OB". reflexivity.
  Case "n = T b". 
    simpl. rewrite twice_reverse_normality.
      reflexivity. rewrite H. reflexivity.
  Case "n = ST b".
    simpl. rewrite IHb. 
      SCase "conclusion". destruct b. reflexivity. reflexivity. reflexivity.
      SCase "assumption". rewrite <- H. reflexivity.
Qed.

(*Theorem normalize_incr_comm : 
  forall n : binary, normalize (incr n) = incr (normalize n).
Proof.
  intros n.
  induction n as [|b|b].
  Case "n = B0". reflexivity.
  Case "n = T b".
    simpl. remember (isPositive b) as ip. destruct ip.
    reflexivity. simpl.*)

  

(*Theorem positive_normalize : 
  forall k : binary,
  (isNormalized (normalize k) = true -> isNormalized  (normalize (T k)) = true)
  -> isPositive k = true.
Proof.
  intros k H.
  destruct k.
  Case "k = OB". rewrite <- H. reflexivity.*)
  

(*Theorem normalize_normalized 
  : forall n : binary, isNormalized (normalize n) = true.
Proof.
  intros n.
  induction n as [|b|b].
  Case "n = OB". reflexivity.
...
Theorem incr_normalized : 
  forall (n : binary), normalize (incr (normalize n)) = incr (normalize n).
Proof.
  intros n.
  induction n as [|b|b].
  Case "n = OB". reflexivity.
...
Theorem from_nat_normalized : 
  forall (n : nat), normalize (from_nat n) = from_nat n.
Proof.
  intros n.
  induction n as [|k].
  Case "n = 0". reflexivity.
...
Theorem nat_iso_r : forall (n : binary), from_nat (to_nat n) = normalize n.
Proof.
  intros n.
  induction n as [|n'|n'].
  Case "n = 0". reflexivity.

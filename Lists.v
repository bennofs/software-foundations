...
Theorem surjective_pairing_stuck : forall (p : natprod),
  p = (fst p, snd p).
Proof.
  simpl. intro p. simpl. destruct p. (* Doesn't reduce anything! *)
Abort.
...
Example test_add2:                count 5 (add 1 [1;4;1]) = 0. reflexivity. Qed.

Definition member (v:nat) (s:bag) : bool := ble_nat 1 (count v s).
Example test_member1:             member 1 [1;4;1] = true.  reflexivity. Qed.
Example test_member2:             member 2 [1;4;1] = false. reflexivity. Qed. 
(** [] *)
...
Theorem add_count_distrib : 
  forall a b : bag, forall v : nat, count v (sum a b) = count v a + count v b.
Proof.
  intros a b v.
  induction a as [|h t].
  Case "a = []". reflexivity.
  Case "a = h :: t". 
    simpl. rewrite IHt.  destruct (beq_nat h v). reflexivity. reflexivity.
Qed. 
...
Theorem snoc_reverse : 
  forall l : natlist, forall n : nat, rev (snoc l n) = n :: rev l.
Proof.
  induction l as [|h t].
  Case "nil". reflexivity.
  Case "cons". simpl. intros n. rewrite IHt. reflexivity.
Qed.
...
Proof.
  ...
  Case "nil". reflexivity.
  Case "cons". 
    simpl. rewrite snoc_reverse. rewrite IHt. reflexivity.
Qed.
...
Proof.
  intros l1 l2.
  induction l1 as [|h t]. 
  Case "nil". simpl.  rewrite app_nil_end. reflexivity.
  Case "cons". simpl. rewrite IHt. simpl. rewrite snoc_append.
...
Fixpoint beq_natlist (l1 l2 : natlist) : bool :=
  match l1, l2 with
    | [], [] => true
    | x :: t1, y :: t2 => andb (beq_nat x y) (beq_natlist t1 t2)
    | _, _ => false
  end.

Example test_beq_natlist1 :   (beq_natlist nil nil = true). reflexivity. Qed.
Example test_beq_natlist2 :   beq_natlist [1;2;3] [1;2;3] = true. reflexivity. Qed.
Example test_beq_natlist3 :   beq_natlist [1;2;3] [1;2;4] = false. reflexivity. Qed.
...
Proof.
  ...
  induction l as [|h t].
  Case "nil". reflexivity.
  Case "cons". simpl. rewrite <- IHt. rewrite <- beq_nat_refl. reflexivity.
Qed.
(** [] *)
...
(** Design exercise: 
     ...
     - Prove it. *) 
Theorem list_design : 
  forall xs ys : natlist, forall a : nat, snoc xs a ++ ys = xs ++ a :: ys.
Proof.
  intros xs ys a.
  rewrite snoc_append. rewrite app_assoc. reflexivity.
Qed.
...
Example test_hd_opt1 : hd_opt [] = None. reflexivity. Qed.
Example test_hd_opt2 : hd_opt [1] = Some 1. reflexivity. Qed.
Example test_hd_opt3 : hd_opt [5;6] = Some 5. reflexivity. Qed.
(** [] *)
